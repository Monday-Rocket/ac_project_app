...
1. **현재 진행 상황**: P1 + P2 코드 레벨 구현 완료 (2026-04-21). 결제(RevenueCat) 연동만 남음.
2. **다음 할 일**:
   - Supabase Dashboard에서 `migration 004` 수동 실행 (실유저 데이터 0건 확인 후)
   - 실기기에서 폴더 UI 시각 검증 (드릴다운/트리 모달/폴더 이동)
   - 실 Apple ID로 크롬 확장 로그인 E2E 검증
   - 결제 붙이고 싶으면 섹션 5 참고 후 C-1 ~ C-3 (RevenueCat)
3. **환경 전제**:
...
| ID | Phase | 제목 | 상태 |
|---|---|---|---|
| #11 | P0 | 공유 폴더 코드 잔재 제거 | **완료** (2026-04-21) |
| #12 | P1 | 로컬 SQLite 스키마 v2 — 중첩 폴더 | **완료** (2026-04-21) |
| #13 | P1 | LocalFolderRepository 계층 조회 메서드 확장 | **완료** (2026-04-21) |
| #17 | P1 | 미분류 폴더 시스템 폴더화 | **완료** (2026-04-21) |
| #14 | P1 | 폴더 리스트 UI — 드릴다운 + 트리 모달 | **완료** (2026-04-21, 실기기 시각 검증 필요) |
| #15 | P1 | 폴더 선택 모달 (검색 + 최근 + 드릴다운) | **완료** (2026-04-21, 실기기 시각 검증 필요) |
| #16 | P1 | 폴더 옵션 메뉴에 "이동" 추가 | **완료** (2026-04-21) |
| #4 | P2 | Supabase schema 정리 (migration 004) | **완료** (2026-04-21, SQL 파일 — **수동 실행 필요**) |
| #5 | P2 | SyncRepository 리팩토링 (백업/복구 API) | **완료** (2026-04-21) |
| #6 | P2 | AuthCubit plan 캐싱 + 만료 감지 | **완료** (2026-04-21) |
| #7 | P2 | Pro CRUD 원격 쓰기 훅 | **완료** (2026-04-21) |
| #8 | P2 | dirty 플래그 기반 보정 백업 | **완료** (2026-04-21) |
| #9 | P2 | MyPage 백업/복구 UI | **완료** (2026-04-21) |
| #10 | P2 | 새 기기 자동 복구 팝업 | **완료** (2026-04-21) |
| #18 | P2 선행 | 크롬 확장 Apple 로그인 추가 (별도 저장소) | **완료** (2026-04-21, 코드 구현 — 실 Apple ID 수동 E2E는 별도 검증) |
